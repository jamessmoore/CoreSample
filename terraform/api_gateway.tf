...
resource "aws_vpc_security_group_egress_rule" "vpc_link_to_alb_iam_audit" {
  security_group_id            = aws_security_group.vpc_link.id
  referenced_security_group_id = aws_security_group.alb.id
  from_port                    = 8003
  to_port                      = 8003
...
# One dedicated integration per backend (not one shared integration relying
# on the ALB's own path-based rules) -- each strips its route's static
# prefix before forwarding, so the backend always sees plain "/mcp"
...
resource "aws_apigatewayv2_integration" "iam_audit_mcp" {
  api_id             = aws_apigatewayv2_api.mcp.id
  integration_type   = "HTTP_PROXY"
  integration_method = "ANY"
  connection_type    = "VPC_LINK"
  connection_id      = aws_apigatewayv2_vpc_link.mcp.id
  integration_uri    = aws_lb_listener.iam_audit_mcp.arn
...
resource "aws_apigatewayv2_route" "iam_audit_mcp" {
  api_id             = aws_apigatewayv2_api.mcp.id
